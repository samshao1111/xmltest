...
/*	CRect rcView;
	...
	CXmlreadView::GetClientRect(rcView);

	if(pDoc->m_xml_file_data.GetLength()>=1)
	{
	pDC->SetBkMode(TRANSPARENT);

	rcView.OffsetRect (1,1);
	pDC->SetTextColor(RGB (0,0,0));
	pDC->DrawText(pDoc->m_xml_file_data, pDoc->m_xml_file_data.GetLength(),rcView, DT_CENTER | DT_CALCRECT);
	}*/
}
...
//pDC->TextOut(40,3,"abcd");
/*
	if(pDoc->m_xml_file_data.GetLength()>=1)
	{

	pDC->TextOut(40,40,pDoc->m_xml_file_data);
	CRect rcView;
	CXmlreadView::GetClientRect(rcView);
